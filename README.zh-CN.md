...
- OpenClaw 参考运维现在使用 `1+3` 每日采集开关：一个总开关，加上午、下午、晚上三个 slot 子开关。网页发布从 slot 状态推导，不再写死暂停状态。
- 默认实例已验证为上午采集开启、下午/晚上暂停。2026-06-11 上午真实运行已保存到 Obsidian，并完成飞书、微信双通道推送。
- `summarize-pro` 应走上游动态模型路由。参考路由会拒绝泄漏思考过程或字符计数的输出，并在网页或反馈摘要消费前干净 fallback。
