...
The 2026-07-09 read-only builder feed canary produced:
...
| Source health summary | all zero |
| Warning | `builder_feed_candidate_absent` for a manifest builder absent from the current feed |
| Live Artifact Fidelity | `passed` |
| Side effects | `file_written=false`, `channel_sent=false`, `side_effects_executed=false` |
...
Live Artifact Fidelity now also emits source-health evidence. The source-health summary distinguishes `controlled_execution_blocked`, `network_failure`, `http_not_ok`, `content_unparseable`, `hash_unavailable`, and `unsupported_adapter`, so `artifact_hash_missing` can be traced to an actual source-health class before any retry/degrade/disable decision is considered. Healing consumes only the summary and still executes nothing.
...
The Collector Execution Policy seam now runs before live adapter dispatch. It allows native read-only adapters (`aggregator_api`, `builder_feed`, `builder_podcast`, `html`, `manual_seed`, `rss`, and YouTube official-feed `video`) while default-blocking controlled adapters that need external runners or nontrivial discovery: `wechat_discovery`, `wechat_mirror`, and Bilibili `video`. A blocked controlled source records `collector_controlled_execution_disabled` with `subprocess_allowed=false`, `browser_allowed=false`, `file_write_allowed=false`, `channel_send_allowed=false`, `cache_write_allowed=false`, and `network_used=false`; it creates no raw artifact and no candidate. The 2026-07-09 read-only WeChat discovery canary selected 3 manifest profiles and produced 3 controlled blocks, with source health classified as `controlled_execution_blocked` rather than `network_failure`. The final scoped CodeRabbit review raised 0 issues.
