...
The Collector Execution Policy seam now runs before live adapter dispatch. It allows native read-only adapters (`aggregator_api`, `builder_feed`, `html`, `manual_seed`, `rss`, and YouTube official-feed `video`) while default-blocking controlled adapters that need external runners or nontrivial discovery: `wechat_discovery`, `wechat_mirror`, `builder_podcast`, and Bilibili `video`. A blocked controlled source records `collector_controlled_execution_disabled` with `subprocess_allowed=false`, `browser_allowed=false`, `file_write_allowed=false`, `channel_send_allowed=false`, `cache_write_allowed=false`, and `network_used=false`; it creates no raw artifact and no candidate. The 2026-07-09 read-only WeChat discovery canary selected 3 manifest profiles and produced 3 controlled blocks, with source health classified as `controlled_execution_blocked` rather than `network_failure`. The final scoped CodeRabbit review raised 0 issues.
